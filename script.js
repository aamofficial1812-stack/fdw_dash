...
function renderTable(data) {
  ...
  rows.forEach((c) => {
    ...
    // ── Row highlight based on follow-up date ──
    if (fu) {
      if (fu < today)      tr.classList.add('overdue');
      else if (fu === today) tr.classList.add('today');
      else                 tr.classList.add('upcoming');
    }

    tr.innerHTML =
      '<td data-label="Name"><strong>' + escHtml(c.name || '') + '</strong></td>' +
      '<td data-label="Phone">' + escHtml(c.phone || '') + '</td>' +
      '<td data-label="Status">' + statusBadge(c.status || '') + '</td>' +
      '<td data-label="Follow-up">' + followupBadge(fu) + '</td>' +
